...
/*
Этот код реализует алгоритм нахождения минимального пути в сетке размера N×M 
с учетом возможных перемещений по диагонали, которые могут быть запрещены для некоторых клеток.

Создается массив dist размером (N+1)×(M+1) для хранения минимальных расстояний до каждой клетки. 
Все элементы инициализируются значением INF

Начальная точка (0,0) устанавливается на расстояние 0.
Определяются возможные направления перемещения:
(1, 0, 100): движение на восток с затратами 100.
(0, 1, 100): движение на север с затратами 100.
(1, 1, 100 * Math.Sqrt(2)): движение по диагонали с затратами 100 * sqrt(2)

Используется SortedSet для хранения элементов с сортировкой по расстоянию. 
Начальная точка (0,0) добавляется в очередь с расстоянием 0.

Цикл продолжается, пока есть элементы в приоритетной очереди:
Извлекается элемент с минимальным расстоянием.
Если текущее расстояние больше уже известного минимального расстояния для этой клетки, цикл продолжается.

Для каждого направления перемещения:
Вычисляются новые координаты (nx,ny).
Если новые координаты выходят за границы сетки, пропускается текущая итерация.
Вычисляется новое расстояние newDist.

Проверка на допустимость диагонального перемещения:
Если направление диагональное и новое положение не разрешено в diagonals, пропускается текущая итерация.

Обновляется минимальное расстояние и элемент добавляется в приоритетную очередь, если новое расстояние меньше уже известного.
*/